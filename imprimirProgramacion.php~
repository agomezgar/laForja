...
//alert(materia+", "+curso+", "+trimestre);
         ...
         $.ajax({
      url:"procesaProgramacion.php",
      type: "POST",
      data: {curso: curso,materia: materia, trimestre: trimestre},
      success: function(opciones){
        $("#tablaProgramacion").html(opciones);
...
$(document).on('click', '#pdfProgramacion', function() {

//alert("Materia: "+materia+", grupo: "+grupo+", curso: "+curso+", trimestre: "+trimestre+", alumno: "+alumno);
cadena='<form method="post" action="dameProgramacionPdf.php" target="_blank">';
...
//alert(cadena);
//$('body').append(cadena)
//$(cadena).appendTo('body');
$(cadena).appendTo(document.body).submit();
});
